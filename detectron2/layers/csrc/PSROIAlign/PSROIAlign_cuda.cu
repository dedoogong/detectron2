...
__global__ void PSRoIAlignForward(
    ...
    const int sampling_ratio, const T* bottom_rois, const int group_size, T* top_data,
...
__device__ void bilinear_interpolate_gradient(
  ...
  if (x_low >= width - 1) {    x_high = x_low = width - 1;
                               x = (T)x_low;
  } else {    x_high = x_low + 1;  }
...
at::Tensor PSRoIAlign_forward_cuda(
  ...
  auto output_dim  = channels / group_size / group_size;
  ...
  AT_DISPATCH_FLOATING_TYPES(input.scalar_type(), "PSRoIAlign_forward", [&] {
...
at::Tensor PSRoIAlign_backward_cuda(
  ...
  AT_DISPATCH_FLOATING_TYPES(grad.scalar_type(), "PSRoIAlign_backward", [&] {
