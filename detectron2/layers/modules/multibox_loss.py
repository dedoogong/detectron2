...
class MultiBoxLoss(nn.Module):
    ...
    def forward(self, predictions, targets, masks, num_crowds): # wrapper, wrapper_mask):
        """Multibox Loss
        Args:
        ...
        if self.cfg.mask_type == config.mask_type.lincomb:
        ...
        defaults = priors.data.detach().cpu().data

        if self.cfg.use_class_existence_loss:
            class_existence_t = loc_data.new(batch_size, num_classes-1)

        for idx in range(batch_size):
            truths      = targets[idx][:, :-1].data.detach().cpu().data
            labels[idx] = targets[idx][:, -1].data.detach().cpu().data.long()
        ...
        if self.cfg.train_boxes:
            ...
            loc_p_min=loc_p.min().data.detach().cpu().data
            loc_p_max=loc_p.max().data.detach().cpu().data
            if loc_p_min<-2e1:
                print('error!! loc_p_min : ',loc_p_min)
            #else:
            #    print('loc_p_min : ',loc_p_min)
            if loc_p_max>2e1:
                print('error!! loc_p_max : ',loc_p_max)
            #else:
            #    print('loc_p_max : ',loc_p_max)
            #print("loc_p min/max: ", loc_p.min().data.detach().cpu().data,loc_p.max().data.detach().cpu().data)
            #print("loc_t min/max: ", loc_t.min().data.detach().cpu().data,loc_t.max().data.detach().cpu().data)##
            losses['B'] = F.smooth_l1_loss(loc_p, loc_t, reduction='sum') * cfg.bbox_alpha

        if self.cfg.train_masks:
            if self.cfg.mask_type == config.mask_type.direct:
                if self.cfg.use_gt_bboxes:
                    ...
                    print("masks_p min/max: ", masks_p.min().data.detach().cpu().data,masks_p.max().data.detach().cpu().data)
                    ##print("masks_t min/max: ", masks_t.min().data.detach().cpu().data,masks_t.max().data.detach().cpu().data)
            ...
            elif self.cfg.mask_type == config.mask_type.lincomb:
        ...
        if self.cfg.use_focal_loss:
            ...
            ##print("before ohem_conf_loss conf_data min/max: ", conf_data.min().data.detach().cpu().data,conf_data.max().data.detach().cpu().data)
            ##print("before ohem_conf_loss conf_t min/max: ", conf_t.min().data.detach().cpu().data,conf_t.max().data.detach().cpu().data)
            losses['C'] = self.ohem_conf_loss(conf_data, conf_t, pos, batch_size)
        # These losses also don't depend on anchors
        if self.cfg.use_class_existence_loss:

            ##print('predictions[classes] min/max: ', predictions['classes'].min().data.detach().cpu().data, predictions['classes'].max().data.detach().cpu().data)
            ##print('class_existence_t min/max: ', class_existence_t.min().data.detach().cpu().data, class_existence_t.max().data.detach().cpu().data)
        ...
        #print('losses_C : ', losses['C'].data.detach().cpu().data, 'losses_B : ', losses['B'].data.detach().cpu().data, 'losses_M : ', losses['M'].data.detach().cpu().data)##
        # Divide all losses by the number of positives.
        # Don't do it for loss[P] because that doesn't depend on the anchors.
        total_num_pos = num_pos.data.detach().cpu().data.sum().float()
        for k in losses:
            if k not in ('P', 'E', 'S'):
        ...
        for idx in range(batch_size):
            ...
            with torch.no_grad():
                ...
                for obj_idx in range(downsampled_masks.size(0)):
                    ...
                    #print('segment_t min/max: ', segment_t[cur_class_t[obj_idx]].min().data.detach().cpu().data, segment_t[cur_class_t[obj_idx]].max().data.detach().cpu().data )
                    #print('downsampled_masks min/max: ', downsampled_masks[obj_idx].min().data.detach().cpu().data, downsampled_masks[obj_idx].max().data.detach().cpu().data )

                    segment_t[cur_class_t[obj_idx]] = torch.clamp(torch.max(segment_t[cur_class_t[obj_idx]], downsampled_masks[obj_idx]), max=1e1)
    ...
    def ohem_conf_loss(self, conf_data, conf_t, pos, num):
        ...
        '''
        ##batch_conf_max=batch_conf.max().data.detach().cpu().data
        ##batch_conf_min=batch_conf.min().data.detach().cpu().data

        if batch_conf_max>1e5:
            ##print('error!! batch_conf_max: ', batch_conf_max)
        else:
            ##print('batch_conf_max: ', batch_conf_max)
        if batch_conf_min<-1e5:
            ##print('error!! batch_conf_min: ', batch_conf_min)
        else:
            ##print('batch_conf_min: ', batch_conf_min)
        '''
        if self.cfg.ohem_use_most_confident:
            ...
            loss_c = log_sum_exp(batch_conf) - batch_conf[:, 0]#####tttT
        ...
        conf_p_max=conf_p.max().data.detach().cpu().data
        conf_p_min=conf_p.min().data.detach().cpu().data

        if conf_p_max>5e1:
            print('error!! conf_p_max: ', conf_p_max)
        #else:
        #    pass#print('conf_p_max: ', conf_p_max)
        if conf_p_min<-5e1:
            print('error!! conf_p_min: ', conf_p_min)
        #else:
        #    pass#print('conf_p_min: ', conf_p_min)
        '''
        #print('conf_p min/max: ', conf_p.min().data.detach().cpu().data, conf_p.max().data.detach().cpu().data)#################
        print('targets_weighted min/max: ', targets_weighted.min().data.detach().cpu().data, targets_weighted.max().data.detach().cpu().data)
        '''
    ...
    def direct_mask_loss(self, pos_idx, idx_t, loc_data, mask_data, priors, masks):
        ...
        for idx in range(mask_data.size(0)):
            with torch.no_grad():
                ...
                pos_bboxes = decode(loc_data[idx, :, :], priors.data.detach().cpu().data, self.cfg.use_yolo_regressors)
    ...
    def lincomb_mask_loss(self, pos, idx_t, loc_data, mask_data, priors, proto_data, masks, gt_box_t, inst_data, interpolation_mode='bilinear'):
        ...
        for idx in range(mask_data.size(0)):
            ...
            #print("pred_masks min/max: ", pred_masks.min().data.detach().cpu().data,pred_masks.max().data.detach().cpu().data)
            if self.cfg.mask_proto_double_loss:
                if self.cfg.mask_proto_mask_activation == config.activation_func.sigmoid:
            ...
            if self.cfg.mask_proto_crop:
                ...
                #print("cropeed pred_masks min/max: ", pred_masks.min().data.detach().cpu().data,pred_masks.max().data.detach().cpu().data)

            if self.cfg.mask_proto_mask_activation == config.activation_func.sigmoid:
                #print("pred_masks min/max: ", pred_masks.min().data.detach().cpu().data,pred_masks.max().data.detach().cpu().data)
                #print("mask_t min/max: ", mask_t.min().data.detach().cpu().data,mask_t.max().data.detach().cpu().data)
                pre_loss = F.binary_cross_entropy(torch.clamp(pred_masks, 0, 1), mask_t, reduction='none')

            else:
                ###print("smooth_l1_loss pred_masks min/max: ", pred_masks.min().data.detach().cpu().data,pred_masks.max().data.detach().cpu().data)
                ###print("smooth_l1_loss mask_t min/max: ", mask_t.min().data.detach().cpu().data,mask_t.max().data.detach().cpu().data)
            ...
            #print("final pre_loss min/max: ", pre_loss.min().data.detach().cpu().data,pre_loss.max().data.detach().cpu().data)
